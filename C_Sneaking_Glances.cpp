...
namespace solve
{
    ...
    void dfs(int step, int pos, int crosses)
    {
        if (step == n)
        {
            max_ans = max(max_ans, crosses);
            return;
        }
        int next_pos1 = pos + a[step];
        int cross1 = crosses + ((pos > 0 && next_pos1 < 0) || (pos < 0 && next_pos1 > 0) ? 1 : 0);
        dfs(step + 1, next_pos1, cross1);
        int next_pos2 = pos - a[step];
        int cross2 = crosses + ((pos > 0 && next_pos2 < 0) || (pos < 0 && next_pos2 > 0) ? 1 : 0);
        dfs(step + 1, next_pos2, cross2);
    }
