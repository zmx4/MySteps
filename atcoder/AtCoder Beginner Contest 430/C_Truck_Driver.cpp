...
constexpr int N = 3e5 + 10, M = 1e5 + 10;
vector<int> prefixA(N), prefixB(N);
inline void solve()
{
    int n, a, b;
    cin >> n >> a >> b;
    ...
    for(int i = 1; i <= n; i++)
    {
        prefixA[i] = prefixA[i - 1] + (s[i] == 'a' ? 1 : 0);
    ...
    for (int i = 1; i <= n;i++) 
    {
        ...
        if(q < i)
            q = i;
        while(p <= n&&prefixA[p] - prefixA[i - 1] < a)
            p++;
        while(q <= n&&prefixB[q] - prefixB[i - 1] < b)
            q++;
        if(p > n)continue;
        ans += max(0ll, q - p);
    }
