...
void pre(string &mid,string &post)
{
    char root = post.back();
    int i = mid.find(root);
    if (i == string::npos) return; // 如果找不到根节点，直接返回
    cout<<root;
    string left_mid = mid.substr(0, i);
    string right_mid = mid.substr(i + 1);
    string left_post = post.substr(0, i);
    string right_post = post.substr(i, post.size() - i - 1);
    pre(left_mid, left_post);
    pre(right_mid, right_post);
}
inline void solve()
{
	string mid,post;
