...
inline void solve()
{
    ...
    int n = v.size();
    int maxlen = 0;
    for(const auto& str : v) maxlen = max(maxlen, (int)str.size());
    bool flag = true;
    ...
    {
        cout << "*";
        int spaces = maxlen - v[i].size();
        if (spaces % 2 == 0) {
            cout << string(spaces / 2, ' ') << v[i] << string(spaces / 2, ' ');
        } else {
            if(flag)
            {
                cout << string(spaces / 2, ' ') << v[i] << string(spaces / 2 + 1, ' ');
                flag = false;
            }
            else
            {
                cout << string(spaces / 2 + 1, ' ') << v[i] << string(spaces / 2, ' ');
                flag = true;
            }
        }
        cout << "*" << endl;
