...
int a[N],b[N],c[N],d[N];
bool vis[10];
void dfs(int x,int y,int z,double now)
{
    if(z == n)
    {
        ans = min(ans, now);
    ...
    for(int i = 1; i <= n; ++i)
    {
        ...
        {
            vis[i] = true;
            double b1 = std::sqrt((x - a[i]) * (x - a[i]) + (y - b[i]) * (y - b[i]));
            double b2 = std::sqrt((a[i] - c[i]) * (a[i] - c[i]) + (b[i] - d[i]) * (b[i] - d[i]));
            dfs(c[i], d[i], z + 1, now + b1/s + b2/t);
            b1 = std::sqrt((x - c[i]) * (x - c[i]) + (y - d[i]) * (y - d[i]));
            dfs(a[i], b[i], z + 1, now + b1/s + b2/t);
            vis[i] = false;
        }
    }
}
