...
inline void Tick()
{
    int n, m1, m2;
    cin >> n >> m1 >> m2;
    DSU dsu1(2*n), dsu2(2*n);
    vector<int> a(m1 + 1), b(m1 + 1);
    vector<int> c(m2 + 1), d(m2 + 1);
    for (int i = 1; i <= m1; ++i)
    {
        cin >> a[i] >> b[i];
    }
    ...
    {
        int u1 = a[i], v1 = b[i];
        int fu1 = dsu1.find(u1), fv1 = dsu1.find(v1);
        int fu2 = dsu2.find(u1), fv2 = dsu2.find(v1);
        if( fu2 == fv2)
        {
            if (fu1 != fv1)
            {
                dsu1.merge(u1, v1); // 在第二张图中也连通时，才真正在第一张图中合并他们
            }
        }
        else
        {
    ...
    {
        int u2 = c[i], v2 = d[i];
        int fu1 = dsu1.find(u2), fv1 = dsu1.find(v2);
        int fu2 = dsu2.find(u2), fv2 = dsu2.find(v2);
        if(fu1 != fv1)
        {
            ans++;
            dsu1.merge(u2, v2);
